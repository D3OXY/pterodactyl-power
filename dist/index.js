...
const axios_1 = __importDefault(__nccwpck_require__(222));
...
if (!PowerActionOptions.includes(powerAction)) {
    core.setFailed(`Invalid power action: ${powerAction}. Must be one of: ${PowerActionOptions.join(", ")}`);
    process.exit(1);
}
try {
...
axios_1.default.defaults.headers.common["Authorization"] = `Bearer ${API_KEY}`;
...
axios_1.default
    ...
    .catch((err) => {
    core.setFailed(`Error sending power action ${powerAction} to server ${serverID}: ${err}`);
    process.exit(1);
});
